...
// File contracts/libraries/DONSwapLibrary.sol
pragma solidity ^0.8.4;

library DONSwapLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IDONSwapPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        require(amountA > 0, 'DONSwap: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'DONSwap: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, 'DONSwap: INVALID_PATH');
...
library TransferHelper {
    function safeApprove(address token, address to, uint256 value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'DONSwap: APPROVE_FAILED');
    }
    ...
    function safeTransferFrom(address token, address from, address to, uint256 value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'DONSwap: TRANSFER_FROM_FAILED');
    }

    function safeTransferETH(address to, uint256 value) internal {
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = to.call{value: value}(new bytes(0));
        require(success, 'DONSwap: ETH_TRANSFER_FAILED');
    }
}

// File contracts/interfaces/IDONSwapRouter.sol
pragma solidity ^0.8.4;
...
// File contracts/DONSwapRouter.sol
pragma solidity ^0.8.4;
